...
import * as tfconv from '@ohos/tfjs-converter';
import * as tf from '@ohos/tfjs-core';
  ...
  async load() {
    if (this.modelUrl) {
      //this.model = await tfconv.loadGraphModel(this.modelUrl);
        this.model= await tfconv.loadGraphModel('mobilenet_v2')
      // Expect that models loaded by URL should be normalized to [-1, 1]
    } else {
       // console.info('url:'+MODEL_INFO[this.version][this.alpha].url)
      const url = MODEL_INFO[this.version][this.alpha].url;
      console.info(url)
      this.model = await tfconv.loadGraphModel('mobilenet_v2', {fromTFHub: true});
    }
